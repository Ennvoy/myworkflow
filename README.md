...
- **確定性閘門**：git commit+push（走 git-tools skill：智慧分群提交＋安全推送）、`.flow` 狀態寫入、`flow-commit-gate`（先標再 commit）、`flow-state done` 自帶閘門（沒驗不准標完成）、`flow-state wave --compute`（波次拓樸＋worker 逐字投餵）、git 原生 pre-commit 兜底（連不經過 Claude 的 commit 也擋 secrets/垃圾）、verify runner 都是 hook / script / skill 確定性節點，模型不能假裝過關。
...
└── dist/                       # 安裝 payload（裝進 ~/.claude）
    ...
    ├── hooks/                  # flow-commit-gate / flow-spec-gate / flow-auto-gate / flow-session-start / flow-size-check / flow-stall-monitor / flow-stop-gate / flow-precompact / flow-dispatch / flow-design-base-hint / flow-precommit（git 兜底）/ precommit-install / commit-gate-core（Node，跨平台）＋ *.test.mjs＋settings.flow.json；完整清單以 dist/hooks/ 為準
