...
**詞彙表閘門（第一次出現詞彙衝突/多義時，SHALL）**：訪談中第一次出現用詞衝突或多義（如「帳號」指 Customer 還是 User）→ 惰性建立 `specs/CONTEXT.md`：純詞彙表、一詞一行「術語＝定義」、**零實作細節**——之後 plan/build 各階段 worker prompt 會帶上這份詞彙表，堵團隊/agent 各自解讀同一詞的漂移。

**既有 codebase 對照**：專案已有 codebase 時，使用者聲稱的現有行為 SHALL 順手對照程式碼；發現矛盾 → 當場 `AskUserQuestion` 確認，不假設使用者記錯或程式碼過時。
