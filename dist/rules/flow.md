...
- **Hook 自動擋**：`flow-commit-gate`（secrets／驗證垃圾／未 done 點名／繞 pre-commit）、`flow-spec-gate`（裸寫 phase/ledger）、`flow-auto-gate`（自駕三硬擋＋policy.json 預核准）、`flow-stop-gate`（自駕全 [x] 未過 complete-check 不准收工）、`flow-precompact`（壓縮前自動 checkpoint）。
- **Script 閘門（SHALL 跑，exit 2）**：`spec-ready --freeze`（開放問題清零＋lens 收斂＋落凍結分母 req-index）、`mockup-check`、`plan-check`（REQ↔task 覆蓋）、`wave --compute`（波次拓樸＋逐字投餵）、`scope --wave`（檔案越界；git 失敗 fail-closed）、`redteam --wave`（攻擊面真的變成測試）、`journey-check`（禁 mock 假綠；通過落 trace）、`ui-fidelity`（web 類：mockup 定版快照未漂移＋定版 tokens 被實作沿用）、`verify-e2e`/`verify-perf`（pass 證據須實存檔）、`complete-check`（完成謂詞全鏈對賬，含 web 須 journey-check＋ui-fidelity@HEAD）、`done`（自帶閘門＋交付歸零＋journal 歸檔）、`mode auto`（guardrail 未過拒寫）。
