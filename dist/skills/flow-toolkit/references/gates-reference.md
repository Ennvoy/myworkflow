...
- **`flow-state spec-ready`**（凍結前）——`### 開放問題` 段缺失/沒清零、缺 `REQ-`/`REQ-E2E-`/`REQ-PERF-`、placeholder（TODO/待定）、REQ-E2E 缺 journey 結構、PERF N/A 無 perf-waiver 豁免檔就 exit 2。**`--freeze`** 通過才寫 `spec-done`，另對賬：`project-type` 落檔（web 類須過走查台＋`ui-signoff` 定版記錄或 mockup-waiver；非 web 即豁免）＋ **lens 審查收斂**（redteam/consistency 各 ≥2 輪末輪零新發現、docHash==現行文字、findings 全終局——`spec-review`/`review-resolve` 的機讀 ledger（診斷：`diagnose review`），細節 `references/spec-review-loop.md`）。凍結瞬間落 `.flow/trace/req-index.json`（REQ 全集＋requirements hash＋HEAD）＝下游唯一分母；原型存在時另落 `.flow/trace/mockup-index.json`（`specs/ui-mockups/` 文字資產逐檔 hash）＝**mockup 定版分母**——凍結後偷改原型在下一道消費閘門（plan-check/wave/ui-fidelity）被 hash 對賬抓。**ui-signoff 總驗（uiSignoffProblem）**：at 時戳缺失/非 ISO/未來日期、重凍結未嚴格晚於上次凍結（錨點取 journal 與舊 mockup-index 較晚者，journal 被清仍判得出）、或簽名快照的 mockup 指紋 ≠ 現行（簽完再改原型＝沒走查過的內容不得凍結）→ exit 2——mockup 修正 SHALL 原地改 `specs/ui-mockups/`（禁另開副本），每次重定版都要使用者重新走查拍板。**mockup-only 重定版短路**：requirements 與上次凍結逐字相同時，lens 沿用既有同文審查證明（docHash 綁定）、不逼整套重跑；文字有變照舊全量重審。
- **`flow-state mockup-check`**（UI 定版前）——互動原型走查台缺任一 `REQ-E2E-*` 卡、本地連結 404、頁面空殼（無 `app.js`/互動元素）、或**連結/引用 script 逃出 `specs/ui-mockups/`**（外部野生頁不進定版指紋＝改動無人抓，目錄圈地）exit 2。
- **`flow-state plan-check`**（plan 出口）——REQ↔task 覆蓋（每條 REQ 被承接、無幻覺 id）＋tasks.md↔manifest 逐欄一致（含 `mockupPages`）＋requirements hash 對賬＋**mockup 鏈路（原型存在時）**：mockup-index hash 對賬＋design.md「UI 對焦結論」節存在且逐頁提及＋每個原型頁被某 task 的 `mockupPages` 承接（幽靈頁/漏承接點名），過了才落 `plan-check.json`（**原型存在時記 mockupAggHash**——重定版後舊計畫在 complete-check 被抓、逼重跑承接對賬）＋`phase=plan-done`。
...
- **`flow-state ui-compare`**（web 類 verify/ship 宣稱綠前，逐頁）——Evaluator 寫 `.flow/trace/ui-compare/map.json` 後跑 `ui-compare-capture.mjs`（截圖腳本自帶「mockup 未凍結/快照漂移」exit 2 閘門），雙 viewport（1440x900＋390x844）fullPage 截圖存 `.flow/trace/ui-compare/`＋落 `capture.json`；Evaluator 逐頁多模態對照後 `flow-state ui-compare <page> --status <pass|fail|n/a>` 落 `.flow/trace/ui-compare.json`——pass 前提＝capture manifest 實存＋該頁雙邊截圖逐檔實存＋截的是現行凍結版原型＋**截圖時的 HEAD＝當前 HEAD**（堵「不重截、只重跑 pass」讓舊截圖領新版合格證）；fail SHALL 附 `--note`；n/a SHALL 綁實存 decision 且不得跨頁重用。**`complete-check` 逐頁對賬**：mockup-index 每個 `pages/*.html` 都要有記錄、無 fail、判現行凍結版原型、**且 pass 記錄的 HEAD 之後真原始碼沒再改過**（sourceChanged 同款過濾；視覺 pass 後又改實作頁＝舊合格證作廢，重截重判——僅 ship 出口驗，wave 首件檢驗不驗）；豁免走 **ui-compare-waiver** decision（整包降級人工對照，附警告）。
- **`flow-state complete-check`**（ship 出口）——**乾淨工作樹**（真原始碼有未 commit 變動＝全體新鮮度檢查失明，先 commit）＋tasks.md 全 `[x]`＋requirements.md 實存且 hash==凍結分母（**req-index 必須實存**，缺=exit 2）＋逐條 `REQ-E2E-*` pass/n-a＋`REQ-PERF-*` 達標記錄＋plan-check manifest 未漂移＋藍軍 code-review red flag 全終局（或 code-review-waiver）＋**web 類須有當前 HEAD 的 journey-check 記錄**（或 journey-waiver）＋**web 類有互動原型須有當前 HEAD＋現行原型指紋的 ui-fidelity 記錄、且 plan-check 記錄的原型指紋＝現行**（重定版後舊 token audit／舊計畫不得沿用）。**0 條 `REQ-E2E-*`、或 journey-check 掃到 0 個 journey 測試檔（落 `noTests:true`）時須有 `e2e-waiver` decision（附理由）才放行；有 ≥1 條 `REQ-E2E-*` 時該 waiver 無效**。`REQ-E2E-*`/`REQ-PERF-*`/**ui-compare 逐頁 pass** 的證據若記錄 HEAD 之後原始碼又改過 → 擋、要求重驗（與 code-review 新鮮度同款）。全過即落 `.flow/trace/complete-check.json`（Stop hook 認這筆）。
