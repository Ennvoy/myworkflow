...
- **`flow-state complete-check`**（ship 出口）——tasks.md 全 `[x]`＋requirements.md 實存且 hash==凍結分母（**req-index 必須實存**，缺=exit 2）＋逐條 `REQ-E2E-*` pass/n-a＋`REQ-PERF-*` 達標記錄＋plan-check manifest 未漂移＋藍軍 code-review red flag 全終局（或 code-review-waiver）＋**web 類須有當前 HEAD 的 journey-check 記錄**（或 journey-waiver）。全過即落 `.flow/trace/complete-check.json`（Stop hook 認這筆）。
