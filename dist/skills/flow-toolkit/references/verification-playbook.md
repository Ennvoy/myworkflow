...
- **便宜迴圈放寬上限**（lint/type/unit）：失敗 → 自動修 → 重跑，每輪回報改了什麼。**但非無限**：這些命令（含 tsc/lint/build）會被 `flow-stall-monitor` 記帳，**同一失敗連 ≥N 輪即注入 STALL**；自駕（`mode:auto`）下連 ≥N+3 輪由 `flow-auto-gate` PreToolUse 硬擋——「lint 一直紅但每次紅得不一樣」靠去噪指紋仍歸同桶、累得到。
...
- **LoopDetection（確定性化）**：跑命令型的 doom loop（反覆跑失敗 test/build/lint）由 `flow-stall-monitor` 斷路器確定性偵測（見上）。**純編輯、完全不跑命令的迴圈**斷路器接不到 → 同一檔反覆編輯超過 N 次仍 SHALL 自覺強制換策略或 check-in（這段靠自律）。
